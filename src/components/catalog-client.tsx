...
export function CatalogClient({ print = false }: { print?: boolean }) {
  ...
  const chooseCategory = (category: Filter) => { setFilter(category); setCollection(""); };
  const chooseCollection = (category: Filter, selectedCollection?: string) => { setFilter(category); setCollection(selectedCollection ?? ""); setColor(""); setPriceRange("all"); };
  const heroImage = filter === "Jogos Americanos" ? "/produtos/Jogo%20Americano%20Redondo%20Xadrez%20%28Dupla%20Face%29.jpg" : filter === "Porta-guardanapos" ? "/produtos/laco-elegance.png" : "/produtos/laco-charm.png";
  ...
  return <main className="catalog-shell"><header className="catalog-nav catalog-header"><Link className="wordmark" href="/" aria-label="Mesa & Graça"><BrandLogo priority /></Link><label className="header-search"><span aria-hidden="true">⌕</span><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Buscar produtos, coleções ou cores" aria-label="Buscar produtos" /></label><nav aria-label="Navegação principal"><button onClick={() => chooseCategory("Jogos Americanos")}>Jogos americanos</button><button onClick={() => chooseCategory("Porta-guardanapos")}>Porta-guardanapos</button><CollectionsMegaMenu products={products} categories={CATEGORIES} onSelect={chooseCollection} /><a href="https://wa.me/5511977007234" target="_blank" rel="noreferrer">Atendimento <ArrowUpRightIcon /></a></nav></header>
