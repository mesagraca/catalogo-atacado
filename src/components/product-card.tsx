...
export function ProductCard({ product, print = false }: { product: Product; print?: boolean }) {
  ...
  const hasCommerceInfo = Boolean(product.sku) || product.wholesale_price != null;
  return <article className="product-card"><Link className="product-image product-open" href={productUrl} aria-label={`Ver ${product.name}`}>{product.image_url ? <img src={product.image_url} alt={product.name} loading="lazy" decoding="async" /> : <div className="image-placeholder">Imagem em atualização</div>}<span>Ver produto</span></Link><div className="product-info"><div className="product-topline"><span>{product.collection || product.category}</span></div><h3>{product.name}</h3>{hasCommerceInfo && <div className="product-commerce">{product.sku && <small>{product.sku}</small>}{product.wholesale_price != null && <div><span>Atacado</span><strong>{money(product.wholesale_price)}</strong></div>}</div>}
