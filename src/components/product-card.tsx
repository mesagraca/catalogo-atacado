...
export function ProductCard({ product, print = false }: { product: Product; print?: boolean }) {
  ...
  const xadrez = product.variations?.filter(v => v.pattern === "xadrez") ?? []; const lisos = product.variations?.filter(v => v.pattern === "liso") ?? [];
  const href = whatsappUrl({ ...product, color_name: selected ? `${selected.pattern === "xadrez" ? "Xadrez" : "Liso"} ${selected.name}` : product.color_name });
  return <article className="product-card"><div className="product-image">{product.image_url ? <img src={product.image_url} alt={product.name} /> : <div className="image-placeholder">Imagem em atualização</div>}</div><div className="product-info"><div className="product-topline"><span>{product.collection || product.category}</span>{product.sku && <small>{product.sku}</small>}</div><h3>{product.name}</h3>
    ...
    <div className="prices"><div><span>Varejo</span><s>{money(product.retail_price)}</s></div><div><span>Atacado</span><strong>{money(product.wholesale_price)}</strong></div></div>{!print && <a className="whatsapp" target="_blank" rel="noreferrer" href={href}>Consultar disponibilidade <em>↗</em></a>}</div></article>;
}
