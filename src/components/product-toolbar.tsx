...
export function ProductSidebar({ open, onToggle, ...props }: SidebarProps) {
  ...
  return <aside className={`product-sidebar ${open ? "open" : "closed"}`} aria-label="Filtros do catálogo">
    <button className="sidebar-toggle" onClick={onToggle} aria-expanded={open}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M4 7h16M4 12h16M4 17h16" strokeLinecap="round" /><circle cx="9" cy="7" r="1.7" fill="var(--cream)" /><circle cx="15" cy="12" r="1.7" fill="var(--cream)" /><circle cx="11" cy="17" r="1.7" fill="var(--cream)" /></svg><span>{open ? "Filtros" : "Mostrar filtros"}</span><b aria-hidden="true">{open ? "−" : "+"}</b></button>
    {open && <div className="sidebar-content"><div className="sidebar-heading"><p>Encontre sua peça</p>{props.activeFilterCount > 0 && <button onClick={props.onClear}>Limpar tudo</button>}</div><details className="filter-group" open><summary>Categoria <i>⌄</i></summary><div className="filter-options"><button className={props.category === "Todos" ? "active" : ""} onClick={() => props.onCategory("Todos")}>Todos os produtos</button>{props.categories.map(item => <button className={props.category === item ? "active" : ""} onClick={() => props.onCategory(item)} key={item}>{item}</button>)}</div></details><details className="filter-group" open><summary>Coleção <i>⌄</i></summary><div className="filter-options"> <button className={!props.collection ? "active" : ""} onClick={() => props.onCollection("")}>Todas as coleções</button>{props.collections.map(item => <button className={props.collection === item ? "active" : ""} onClick={() => props.onCollection(item)} key={item}>{item}</button>)}</div></details>{props.colors.length > 0 && <details className="filter-group"><summary>Cor <i>⌄</i></summary><div className="filter-options color-options"><button className={!props.color ? "active" : ""} onClick={() => props.onColor("")}>Todas as cores</button>{props.colors.map(item => <button className={props.color === item ? "active" : ""} onClick={() => props.onColor(item)} key={item}>{item}</button>)}</div></details>}<details className="filter-group" open><summary>Preço atacado <i>⌄</i></summary><div className="filter-options"><button className={props.priceRange === "all" ? "active" : ""} onClick={() => props.onPriceRange("all")}>Todos os preços</button><button className={props.priceRange === "up-to-10" ? "active" : ""} onClick={() => props.onPriceRange("up-to-10")}>Até R$ 10</button><button className={props.priceRange === "10-to-15" ? "active" : ""} onClick={() => props.onPriceRange("10-to-15")}>R$ 10 — R$ 15</button><button className={props.priceRange === "over-15" ? "active" : ""} onClick={() => props.onPriceRange("over-15")}>Acima de R$ 15</button></div></details><p className="sidebar-selection">{props.activeFilterCount > 0 ? `${props.activeFilterCount} filtro${props.activeFilterCount > 1 ? "s" : ""} aplicado${props.activeFilterCount > 1 ? "s" : ""}` : `Exibindo: ${activePrice}`}</p></div>}
  </aside>;
}
